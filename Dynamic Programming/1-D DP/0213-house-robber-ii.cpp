...
public:
    ...
    int solve(vector<int> &nums, int i, int n) {
        if(i > n) return 0;                 //similar to houseRobberI but not using equalTo as it is to be included
    ...
    int rob(vector<int>& nums) {
        ...
        memset(t, -1, sizeof(t));
        int robFirst = solve(nums, 0, n-2);        //robbing the first house, hence not considering the last house
        memset(t, -1, sizeof(t));
        int notRobFirst = solve(nums, 1, n-1);     //not robbing the first, house hence considering the last house
